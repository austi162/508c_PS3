...
outreg2 using PS3_Q4a_Outreg.xls, ctitle(Mortality Logit) addtext(Logit,X)append label
predict p_logit
...
outreg2 using PS3_Q4a_Outreg.xls, ctitle(Health Status LPM) addtext(LPM,X)append label
predict b_ols

probit badhealth age edyrs_cat faminc_cat race_cat, r
outreg2 using PS3_Q4a_Outreg.xls, ctitle(Health Status Probit) addtext(Probit,X)append label
predict b_probit

logit badhealth age edyrs_cat faminc_cat race_cat, r
outreg2 using PS3_Q4a_Outreg.xls, ctitle(Health Status Logit) addtext(Logit,X)append label
predict b_logit
...
outreg2 using PS3_Q4b_Outreg.xls, ctitle(Mortality Probit MFX) addtext(Probit,X)replace label
mfx compute

pause

logit mort5 age edyrs_cat faminc_lt20 faminc_20t75 white hisp other, r
outreg2 using PS3_Q4b_Outreg.xls, ctitle(Mortality Logit MFX) addtext(Logit,X)append label
mfx compute

pause

probit badhealth age edyrs_cat faminc_lt20 faminc_20t75 white hisp other, r
outreg2 using PS3_Q4b_Outreg.xls, ctitle(Health Status Probit MFX) addtext(Probit,X)append label
mfx compute 
